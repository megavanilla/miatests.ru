...
Class Main
{
    public function show()
    {
        ...
        //$Auth = new mvc\controllers\Auth('user1');
    ...
    {
        $User = new mvc\controllers\User('user1');
        $Views = new mvc\views\View();
        $userData = $User->getUserFromLogin();
        $uid = (!empty($userData['id']))?$userData['id']:'';
        $uHash = (!empty($userData['hash']))?$userData['hash']:'';

        //setcookie('uid', $uid, time() + 86400, '/', $_SERVER['HTTP_HOST'], false, false);
        setcookie('uih', $uHash, time() + 86400, '/', $_SERVER['HTTP_HOST'], false  , false);
        $Views->showPage('main/getUser', $userData);
    }
