...
class Mail
{
    ...
    */
    ...
    /**
    * Позволяет подписывать письмо
    * **/
    private $sign_cert = false;
    /**
    * Пути к файлу сертификата и его ключу
    * **/
    private $cert_file = '';
    private $cert_key_file = '';
    /**
    * Инициализаця
    ...
    * @param boolean $sign_cert Нужна для подписи письма сертификатом
    * @param string $cert_file путь к файлу сертификату
    * @param string $cert_key_file путь к ключу сертификата
    */
    public function __construct($charset = "", $ctencoding = '', $idna_convert = false, $sign_cert = false, $cert_file = '', $cert_key_file = '')
    {
        /** Устанавливаем название организации */
        $this->Organization('Ltd. NyuEfSi');
        $this->sign_cert = $sign_cert;
        $this->cert_file = $cert_file;
        $this->cert_key_file = $cert_key_file;
        if ($sign_cert === true) {
            /** Подключаем класс для DKIM подписей*/
            require_once (dirname(__file__) . "/mail-signature.class.php"); // подгружаем класс для конвертации
        }
    ...
    */
    public function Body($text, $text_html = "", $alternative_text = '', $resource = 'framework')
    {
        // по умолчанию ресурс framework
        if (!strlen($resource))
        $resource = 'framework';
        if ($text_html == "html")
        $text_html = "text/html";
    ...
    * @return boolean
    */
    public function Attach($filename, $new_name_filename = "", $filetype = "", $disposition = "", $resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
    ...
    */
    public function BuildMail($resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
        ...
        $this->headers["X-Mailer"] = "Ltd. NyuEfSi";
        // если для текущего ресурса тема не установлена, но установлена для ресурса по умолчанию, возьмем тему от туда
        if (!isset($this->headers[$resource]['Subject']) and isset($this->headers['framework']['Subject']))
        $this->headers[$resource]['Subject'] = $this->headers['framework']['Subject'];
    ...
    */
    public function Subject($subject, $resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
    ...
    */
    public function To($to, $resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
        // если это массив
    ...
    */
    public function Cc($cc, $resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
        if (is_array($cc)) {
    ...
    */
    public function Bcc($bcc, $resource = 'framework')
    {
        if (!strlen($resource))
        $resource = 'framework';
        if (is_array($bcc)) {
    ...
    */
    ...
    /**
    * Метод выполняет отправку email сообщения одной строкой,
    * помимо этого, перед отправкой проверяется - разрешено ли отправлять сообщения
    * **/
    public function Send_Def_Param($email = null, $from = null, $subject = null, $msg = null, $resource = "", $attach = null, $zipping = 0, $file_name_archive = null)
    {
        if ($from == null && empty($from))
        {
            $from = NEWFC_FEEDBACK_EMAIL;
        }
        if (is_string($email)) {
            $email = explode(',', $email);
        }
        //Убираем все лишние пробелы в параметрах
        for ($i_email = 0; $i_email < count($email); $i_email++) {
            $email[$i_email] = trim($email[$i_email]);
        }
        if (!is_string($resource))
        {
            $resource = "framework.ru";
        }
        /**
        * Если переданы основные данные для отправки сообщения
        * **/
        if (NEWFC_SEND_EMAIL === true && $email != null && $subject != null && $msg != null && !empty($email) && !empty($subject) && !empty($msg) && (is_string($email) || is_array($email)) && is_string($subject) && is_string($msg)) {
            //var_dump($email);
            //var_dump($resource);
            /**
            * Если переданы файлы до аттачим их в сообщение
            * **/
            /**
            * Если передан ключ архиации файлов, то архивируем их перед отправкой
            * **/
            $count_attach_zip = 0;
            /**
            * Цикл email адресов
            * **/
            //$this->To($email,$resource);

            $res_el           = "";
            for ($i_email_el = 0; $i_email_el < count($email); $i_email_el++) {
                /**
                * Если в адресах есть дубликаты, то будет отправляться только одно письмо. Поэтому можно применять две строки, с разными ресурсами
                */
                $this->To($email[$i_email_el], $resource); // кому, в этом поле так же разрешено указывать имя
                //var_dump($resource);
                //$res_el .= $email[$i_email_el];
                /*if ($i_email_el < count($email) - 1)
                {
                $res_el .= '_';
                }*/
            }

            $this->From($from); // от кого Можно использовать имя, отделяется точкой с запятой
            //$this->To($email); // кому, в этом поле так же разрешено указывать имя
            $this->Subject($subject,$resource); //Тема сообщения
            $this->Body($msg,"","",$resource); //Текст сообщения
            $this->Priority(4); // установка приоритета
            $this->log_on(true); // включаем лог, чтобы посмотреть служебную информацию
            if (is_array($attach) && !empty($attach)) {
                //Если передан массив файлов
                if ($zipping != 1) {
                    for ($i = 0; $i < count($attach); $i++) {
                        //var_dump("Тут просто файлы");
                        $file_name = (array_key_exists('file_name', $attach[$i]) && is_file($attach[$i]['file_name'])) ? $attach[$i]['file_name'] : null;
                        $new_name_filename = (array_key_exists('new_name_filename', $attach[$i]) && !empty($attach[$i]['new_name_filename'])) ? $attach[$i]['new_name_filename'] : null;
                        $new_name_filename = ($file_name != null && $new_name_filename == null) ? pathinfo($file_name, PATHINFO_BASENAME) : $new_name_filename;
                        $file_type = (array_key_exists('file_type', $attach[$i]) && !empty($attach[$i]['file_type'])) ? $attach[$i]['file_type'] : 'application/pdf';
                        /*var_dump(array(
                        $file_name,
                        $new_name_filename,
                        $file_type,
                        print_r($attach)));*/
                        if ($file_name != null && $new_name_filename != null) {
                            $this->Attach($file_name, $new_name_filename, $file_type, "attachment", $resource);
                        }
                    }
                }//Если включена архивация
                else
                {
                    $date = new DateTime();
                    if ($file_name_archive != null && !empty((string )$file_name_archive)) {
                        $file_name_archive = (string )$file_name_archive . '_' . (string )$date->format('j_m_Y_h_i_s_u') . ".zip";
                    }
                    else
                    {
                        $file_name_archive = (string )$date->format('j_m_Y_h_i_s_u') . ".zip";
                    }
                    $file_path_arcive = NEWFC_DIR_TREATMENTS . $file_name_archive;
                    //Создаем zip архив с документами
                    $zip              = new ZipArchive();
                    $zip_open         = ($zip->open($file_path_arcive, ZIPARCHIVE::CREATE) === true) ? true : false;
                    for ($i = 0; $i < count($attach); $i++) {
                        //var_dump("Тут zip файлы");
                        $file_name = (array_key_exists('file_name', $attach[$i]) && is_file($attach[$i]['file_name'])) ? $attach[$i]['file_name'] : null;
                        $new_name_filename = (array_key_exists('new_name_filename', $attach[$i]) && !empty($attach[$i]['new_name_filename'])) ? $attach[$i]['new_name_filename'] : null;
                        $new_name_filename = ($file_name != null && $new_name_filename == null) ? pathinfo($file_name, PATHINFO_BASENAME) : $new_name_filename;
                        $file_type = (array_key_exists('file_type', $attach[$i]) && !empty($attach[$i]['file_type'])) ? $attach[$i]['file_type'] : 'application/pdf';
                        ...
                        $file_type,
                        print_r($attach)));*/
                        if ($file_name != null && $new_name_filename && $zip_open == true) {
                            $zip->addFile($file_name, $new_name_filename);
                            $count_attach_zip++;
                        }

                    } //End for $i = 0; $i < count($attach); $i++
                    //закрываем архив
                    $zip->close();
                    if ($count_attach_zip > 0) {
                        $this->Attach($file_path_arcive, $file_name_archive, $file_type, "attachment", $resource);
                    }
                }//Если отключена архивация
            } //Если передан массив файлов

            if ($this->Send() != false)
            {
                //print('Отправлено сообщение с ресурсом: '.$resource."\r\n");
                $this->sendto = array();
                $this->smtpsendto = array(); // ключи и значения одинаковые, чтобы исключить дубли адресов
                $this->names_email = array(); // имя первая часть
            }
            //return true;
        } //Если разрешена отправка почты и переданы основные данные для отправки сообщения
        else
        {
            return false;
        }
    ...
    public function Send()
    {
        ...
        if (!$this->smtp['on']) {
            ...
            foreach ($this->sendto as $key => $value) {
                ...
                $body_resource = $key;
                // а если тело для текущего ресурса не софрмированно, будем работать с телом от ресурса по умолчанию
                else
                $body_resource = 'framework.ru';
                /**
                * Если включена опция подписи письма, и файлы сертификата и ключа сертификата существуют
                * */
                $signed_headers= '';
                if ($this->sign_cert === true && !empty($this->cert_file) && !empty($this->cert_key_file) && is_file($this->cert_file) && is_file($this->cert_key_file)) {
                    $key_str        = file_get_contents($this->cert_key_file);
                    $signature      = new mail_signature($key_str, '', NEWFC_SITE_DOMAIN_NAME, 'framework_selector');
                    $signed_headers = $signature->get_signed_headers($strTo, $this->headers[$key]['Subject'], $this->body[$body_resource], $this->ready_headers[$key]);
                }
                ...
                $res = @mail($strTo, $this->headers[$key]['Subject'], $this->body[$body_resource], $signed_headers . $this->ready_headers[$key]);
